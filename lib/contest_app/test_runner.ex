defmodule ContestApp.TestRunner do
  use GenServer

  alias ContestApp.Participants

  @interval :timer.seconds(10)

  # Public Api

  def start_link(_opts) do
    GenServer.start_link(__MODULE__, %{}, name: __MODULE__)
  end

  # GenServer Callbacks

  @impl true
  def init(state) do
    schedule_test()
    {:ok, state}
  end

  @impl true
  def handle_info(:run_tests, state) do
    run_tests_for_particpants()
    schedule_test()
    {:noreply, state}
  end

  # Private Helpers

  defp schedule_test do
    Process.send_after(self(), :run_tests, @interval)
  end

  defp run_tests_for_particpants do
    participants = Participants.list_all()

    Enum.each(participants, fn participant ->
      run_tests_until_fail(participant)
    end)
  end

  defp run_tests_until_fail(participant) do
    tests = ContestApp.Tests.all()

    Enum.reduce_while(tests, nil, fn test_module, _acc ->
      case test_module.run_test(participant.api_url, participant.id) do
        {:ok, result} ->
          IO.puts("""
          Passed test:
          - Test: #{test_module.name()}
          - Participant: #{participant.name} (ID: #{participant.id})
          - Level: #{test_module.level()}
          """)

          {:cont, result}

        {:error, reason} ->
          IO.puts("""
          Failed test:
          - Test: #{test_module.name()}
          - Participant: #{participant.name} (ID: #{participant.id})
          - Level: #{test_module.level()}
          - Reason: #{inspect(reason)}
          """)

          {:halt, reason}
      end
    end)
  end
end
